...
#com_p = (basePos[0]+interval, basePos[1]+interval, basePos[2]+interval)
#p.resetDebugVisualizerCamera( cameraDistance=1, cameraYaw=75, cameraPitch=-20, cameraTargetPosition=com_p) # fix camera onto model
...
def kuka_camera(robotid, eei):
    ...
    com_p, com_o, _, _, _, _ = p.getLinkState(robotid, eei, computeForwardKinematics=True)
    print()
    ...
    view_matrix = p.computeViewMatrix(com_p, com_p + 0.1 * camera_vector, camera_vector)
