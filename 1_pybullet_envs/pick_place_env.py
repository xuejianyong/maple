...
class PickPlaceEnv():
    ...
    def reset(self, config):
        ...
        for obj_name in obj_names:
            ...
            if ("block" in obj_name) or ("bowl" in obj_name) or ("push" in obj_name):
                ...
                #    object_position[2] = 0
                #    object_tname = obj_name.split(" ")[2]
                #    object_id = pybullet.loadURDF("object/"+object_tname+".urdf", object_position, useFixedBase=1)
        ...
        #rand_y = np.random.uniform(BOUNDS[1, 0] + 0.1, BOUNDS[1, 1] - 0.1)
        #rand_xyz = np.float32([rand_x, rand_y, 0.03]).reshape(1, 3)
        #self.cube = pybullet.loadURDF("object/cube.urdf", rand_xyz.squeeze(), useFixedBase=1)  # For forward kinematics.
        #rand_x = np.random.uniform(BOUNDS[0, 0] + 0.1, BOUNDS[0, 1] - 0.1)
        #rand_y = np.random.uniform(BOUNDS[1, 0] + 0.1, BOUNDS[1, 1] - 0.1)
        #rand_xyz = np.float32([rand_x, rand_y, 0.03]).reshape(1, 3)
        #self.cylinder = pybullet.loadURDF("object/cylinder.urdf", rand_xyz.squeeze(), useFixedBase=1)  # For forward kinematics.
        #rand_x = np.random.uniform(BOUNDS[0, 0] + 0.1, BOUNDS[0, 1] - 0.1)
        #rand_y = np.random.uniform(BOUNDS[1, 0] + 0.1, BOUNDS[1, 1] - 0.1)
        #rand_xyz = np.float32([rand_x, rand_y, 0.03]).reshape(1, 3)
        #self.roof = pybullet.loadURDF("object/roof.urdf", rand_xyz.squeeze(), useFixedBase=1)  # For forward kinematics.
