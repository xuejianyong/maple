...
def get_ckpt_update_func(variant):
    print('   *** get_ckpt_update_func() robosuite_lanucher.py')
    import os.path as osp
    import torch
    from maple.launchers.conf import LOCAL_LOG_DIR
    def ckpt_update_func(algo, epoch):
        print('   *** ckpt_update_func() robosuite_lanucher.py')
...
def experiment_check(variant):
    ...
    """
    checking the trained algorithm in new tasks. 
    now, the cube is not redwood anymore, is greenwood
    """
    import os.path as osp
    from maple.launchers.conf import LOCAL_LOG_DIR
    ...
    algorithm.eval_data_collector._policy = eval_policy
    algorithm.expl_data_collector._policy = expl_policy
    ...
    if 'ckpt_path' in variant:
        print('   *** get ckpt_func from robosuite_launcher.py')
        ckpt_update_func = get_ckpt_update_func(variant)
        algorithm.pre_epoch_funcs.insert(0, ckpt_update_func)
...
def experiment_sim(variant):
    ...
    if action_dim_s == 0:
        ...
        policy = policy_class(  # PAMDPPolicy
            obs_dim=obs_dim,
            action_dim_s=action_dim_s,
            action_dim_p=action_dim_p,
            hidden_sizes=[M, M],
